...
def Tk_get_root():
    if not hasattr(Tk_get_root, "root"): #(1)
        ...
        Tk_get_root.root.geometry("1280x960+5+5")
        Tk_get_root.root.resizable(False, False)
        #Tk_get_root.root.configure(background='black')
...
class LED(Tk.Frame):
    ...
    def __init__(self, parent):
        ...
        LED_OFF  = ttk.Button(ctrlpanel, text="OFF")
        LED_ON   = ttk.Button(ctrlpanel, text="ON")
        LED_1   = ttk.Button(ctrlpanel, text="LED 1")
        LED_2   = ttk.Button(ctrlpanel, text="LED 2")
        LED_3   = ttk.Button(ctrlpanel, text="LED 3")
...
class Orientation(Tk.Frame):
    ...
    def __init__(self, parent, canvas):
        ...
        xz = self.canvas.create_oval(60, 285, 210, 435,outline= "black" ,fill="white")
        xy = self.canvas.create_oval(240, 285, 390, 435,outline= "black" ,fill="white")
        ...
        abovex = self.canvas.create_line(60, 360, 210, 360)
        abovey = self.canvas.create_line(135, 285, 135, 435)
        sidex = self.canvas.create_line(240, 360, 390, 360)
        sidez = self.canvas.create_line(315,285,315,435)

        text = "Y"
        title = Tk.Text(self.parent, height=1, width=1, relief=Tk.FLAT, bg=self.parent.cget('bg'))
        title.insert(Tk.END, text)
        title.place(x=950, y=742)

        text = "X"
        title = Tk.Text(self.parent, height=1, width=1, relief=Tk.FLAT, bg=self.parent.cget('bg'))
        title.insert(Tk.END, text)
        title.place(x=860, y=830)

        text = "Z"
        title = Tk.Text(self.parent, height=1, width=1, relief=Tk.FLAT, bg=self.parent.cget('bg'))
        title.insert(Tk.END, text)
        title.place(x=770, y=742)

        ctrlpanel = ttk.Frame(self.parent)

        UPDATE = ttk.Button(ctrlpanel, text="UPDATE")

        UPDATE.pack(side=Tk.BOTTOM)

        ctrlpanel.place(x=827.5, y=917.5)
...
class SIGNAL(Tk.Frame):
    ...
    def __init__(self, parent):
        ...
        SIGNAL  = ttk.Button(ctrlpanel, text="SIGNAL")

        SIGNAL.pack(side=Tk.BOTTOM)

        ctrlpanel.place(x=1120, y=917.5)
...
if __name__ == "__main__":
    ...
    c.create_rectangle(45, 5, 635, 475,outline= "black" ,fill="white")
    c.create_rectangle(50, 10, 295, 175,outline= "black" ,fill=root.cget('bg'))
    c.create_rectangle(50, 180, 400, 255,outline= "black" ,fill=root.cget('bg'))
    c.create_rectangle(300, 10, 400, 175,outline= "black" ,fill=root.cget('bg'))
    c.create_rectangle(50, 260, 400, 470,outline= "black" ,fill=root.cget('bg'))
    c.create_rectangle(405, 180, 630, 470,outline= "black" ,fill=root.cget('bg'))
    c.create_rectangle(430, 210, 605, 420,outline= "black" ,fill="black")

    c.create_image(520,90, image=img1)
    ...
    accmeter = Orientation(root, c)

    signal_tester = SIGNAL(root)
