...
while True :
    ...
    #yleinen kulma painovoimaan nahden
    arvo = accl['z']
    if arvo <= 1030 and arvo >= 0:
        kulma = (math.acos(arvo/1030)*180)/math.pi
    elif arvo >= -1015 and arvo < 0:
        kulma = (math.acos(arvo/1015)*180)/math.pi
    else:
        kulma = 'tuntematon' 

    #kulma x-suunnassa
    xarvo = accl['x']
    if xarvo <= 1010 and xarvo >= 0:
        xkulma = -90 + (math.acos(xarvo/1010)*180)/math.pi
    elif xarvo >= -1010 and xarvo < 0:
        xkulma = -90 + (math.acos(xarvo/1010)*180)/math.pi
    else:
        xkulma = 'tuntematon'  
    ...
    #kulma y-suunnassa
    yarvo = accl['y']
    if yarvo <= 1045 and yarvo >= 0:
        ykulma = -90 + (math.acos(yarvo/1045)*180)/math.pi
    elif yarvo >= -1030 and yarvo < 0:
        ykulma = -90 + (math.acos(yarvo/1030)*180)/math.pi
    else:
        ykulma = 'tuntematon'  

    #jos anturin kokema kiihtyvyys on suurempi kuin 1 g, eli 1024, pallo on viel liikkeess,
    #eika todellista asentoa voida viel laskea.
  
    if kulma == 'tuntematon':
        print("Laite on liikkeess")
    else:
        print("Laite on %d asteen kulmassa painovoimaan nhden." %kulma)
    if xkulma == 'tuntematon':
        print("Laite on liikkeess")
    else:
        print("Laite on %d asteen kulmassa x-suunnassa." %xkulma)
    if ykulma == 'tuntematon':
        print("Laite on liikkeess")
    else:
        print("Laite on %d asteen kulmassa y-suunnassa." %ykulma)
